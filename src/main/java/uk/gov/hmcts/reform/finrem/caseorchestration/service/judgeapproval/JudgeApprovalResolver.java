...
class JudgeApprovalResolver {
     ...
     */
    void processApprovableCollection(FinremCaseDetails finremCaseDetails, List<? extends Approvable> approvables, CaseDocument targetDoc, JudgeApproval judgeApproval,
                                     String userAuthorisation) {
        ofNullable(approvables)
            .ifPresent(list ->
                list.forEach(el -> ofNullable(el)
                    .filter(approvable -> approvable.match(targetDoc))
                    .ifPresent(approvable -> handleApprovable(finremCaseDetails, approvable, judgeApproval, userAuthorisation))
                )
            );
    }
...
//                approvable.setCourtOrderDate(judgeApproval.getCourtOrderDate());
    ...
    private void processDraftOrderDocReviewCollection(FinremCaseDetails finremCaseDetails, DraftOrdersWrapper draftOrdersWrapper, CaseDocument targetDoc, JudgeApproval judgeApproval,
                                                      String userAuthorisation) {
