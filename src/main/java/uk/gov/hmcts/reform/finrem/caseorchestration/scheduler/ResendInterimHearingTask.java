...
public class ResendInterimHearingTask extends EncryptedCsvFileProcessingTask {
    ...
    @Value("${cron.resendInterimHearing.caseListFileName:resendlistforhearing-encrypted.csv}")
    ...
    protected void executeTask(FinremCaseDetails finremCaseDetails) {
        ...
        List<InterimHearingCollection> interimHearings = caseData.getInterimWrapper()
            ...
            .filter(hearing -> hearing.getValue().getInterimHearingDate().isAfter(LocalDate.of(2025, 4, 10)))
            .toList();
        ...
        caseData.getInterimWrapper().getInterimHearingDocuments().stream()
            .map(doc -> doc.getValue().getCaseDocument().getUploadTimestamp())
            .max(LocalDateTime::compareTo).ifPresent(latestTimestamp -> caseData.getInterimWrapper().getInterimHearingDocuments().removeIf(doc ->
                doc.getValue().getCaseDocument().getUploadTimestamp().isEqual(latestTimestamp)
            ));
