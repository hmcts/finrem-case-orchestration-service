...
public class IntervenerDocumentService {
    ...
    public CaseDocument generateIntervenerAddedNotificationLetter(FinremCaseDetails finremCaseDetails, String authToken,
        ...
        log.info("Generating Intervener Added Notification Letter {} from {} for bulk print for {}",
            documentConfiguration.getIntervenerAddedTemplate(), documentConfiguration.getIntervenerAddedFilename(), recipient);
        ...
        return getIntervenerAddedCaseDocument(authToken, intervenerAddedLetterDetails);
    }
    ...
    public CaseDocument generateIntervenerRemovedNotificationLetter(FinremCaseDetails finremCaseDetails, String authToken,
        ...
        log.info("Generating Intervener Removed Notification Letter {} from {} for bulk print for {}",
            documentConfiguration.getIntervenerRemovedTemplate(), documentConfiguration.getIntervenerRemovedFilename(), recipient);

        CaseDetails caseDetailsForBulkPrint = documentHelper.prepareIntervenerLetterTemplateData(finremCaseDetails, recipient);
        finremCaseDetails.getData().setCurrentAddressee((Addressee) caseDetailsForBulkPrint.getData().get(ADDRESSEE));
        IntervenerRemovedLetterDetails intervenerRemovedLetterDetails = generateRemovedLetterDetails(finremCaseDetails);

        return getIntervenerRemovedCaseDocument(authToken, intervenerRemovedLetterDetails);
    }
    ...
    public CaseDocument generateIntervenerSolicitorAddedLetter(FinremCaseDetails finremCaseDetails, String authToken,
        ...
        log.info("Generating Intervener Added Solicitor Notification Letter {} from {} for bulk print for {}",
            documentConfiguration.getIntervenerAddedSolicitorTemplate(), documentConfiguration.getIntervenerAddedSolicitorFilename(), recipient);
        ...
        return getIntervenerAddedCaseDocument(authToken, intervenerAddedSolicitorLetterDetails);
    }
    ...
    public CaseDocument generateIntervenerSolicitorRemovedLetter(FinremCaseDetails finremCaseDetails, String authToken,
        ...
        log.info("Generating Intervener Removed Solicitor Notification Letter {} from {} for bulk print for {}",
            documentConfiguration.getIntervenerRemovedSolicitorTemplate(), documentConfiguration.getIntervenerRemovedSolicitorFilename(), recipient);
        ...
        return getIntervenerRemovedCaseDocument(authToken, intervenerRemovedSolicitorLetterDetails);
    }

    private CaseDocument getIntervenerAddedCaseDocument(String authToken, IntervenerAddedLetterDetails intervenerAddedLetterDetails) {
        CaseDocument generatedIntervenerAddedNotificationLetter =
            generateDocument(authToken, intervenerAddedLetterDetails, documentConfiguration.getIntervenerAddedTemplate(),
    ...
    private CaseDocument getIntervenerAddedCaseDocument(String authToken,
                                                        IntervenerAddedSolicitorLetterDetails intervenerAddedSolicitorLetterDetails) {
        CaseDocument generatedIntervenerAddedSolicitorNotificationLetter =
            generateDocument(authToken, intervenerAddedSolicitorLetterDetails, documentConfiguration.getIntervenerAddedSolicitorTemplate(),
    ...
    private CaseDocument getIntervenerRemovedCaseDocument(String authToken, IntervenerRemovedLetterDetails intervenerRemovedLetterDetails) {
        CaseDocument generatedIntervenerRemovedNotificationLetter =
            generateDocument(authToken, intervenerRemovedLetterDetails, documentConfiguration.getIntervenerRemovedTemplate(),
    ...
    private CaseDocument getIntervenerRemovedCaseDocument(String authToken,
                                                          IntervenerRemovedSolicitorLetterDetails intervenerRemovedSolicitorLetterDetails) {
        CaseDocument generatedIntervenerRemovedSolicitorNotificationLetter =
            generateDocument(authToken, intervenerRemovedSolicitorLetterDetails, documentConfiguration.getIntervenerRemovedSolicitorTemplate(),
    ...
    private CaseDocument generateDocument(String authToken, IntervenerAddedLetterDetails intervenerAddedLetterDetails, String template,
                                          String filename) {
        return genericDocumentService.generateDocumentFromPlaceholdersMap(authToken, convertLetterDetailsToMap(intervenerAddedLetterDetails),
            template, filename);
    }

    private CaseDocument generateDocument(String authToken, IntervenerRemovedLetterDetails intervenerRemovedLetterDetails, String template,
                                          String filename) {
        return genericDocumentService.generateDocumentFromPlaceholdersMap(authToken, convertLetterDetailsToMap(intervenerRemovedLetterDetails),
            template, filename);
    }

    private CaseDocument generateDocument(String authToken, IntervenerAddedSolicitorLetterDetails intervenerAddedSolicitorLetterDetails,
                                          String template, String filename) {
        return genericDocumentService.generateDocumentFromPlaceholdersMap(authToken, convertLetterDetailsToMap(intervenerAddedSolicitorLetterDetails),
            template, filename);
    }

    private CaseDocument generateDocument(String authToken, IntervenerRemovedSolicitorLetterDetails intervenerRemovedSolicitorLetterDetails,
                                          String template, String filename) {
        return genericDocumentService.generateDocumentFromPlaceholdersMap(authToken,
            convertLetterDetailsToMap(intervenerRemovedSolicitorLetterDetails), template, filename);
    }

    private IntervenerAddedLetterDetails generateAddedLetterDetails(FinremCaseDetails caseDetails) {

        return IntervenerAddedLetterDetails.builder().courtDetails(CaseHearingFunctions.buildFrcCourtDetails(caseDetails.getData()))
            .addressee(caseDetails.getData().getCurrentAddressee()).divorceCaseNumber(caseDetails.getData().getDivorceCaseNumber())
            .applicantName(caseDetails.getData().getFullApplicantName()).respondentName(caseDetails.getRespondentFullName())
            .letterDate(DateTimeFormatter.ofPattern(LETTER_DATE_FORMAT).format(LocalDate.now())).caseNumber(caseDetails.getId().toString())
            .intervenerFullName(caseDetails.getData().getCurrentIntervenerChangeDetails().getIntervenerDetails().getIntervenerName()).build();
    }

    private IntervenerRemovedLetterDetails generateRemovedLetterDetails(FinremCaseDetails caseDetails) {

        return IntervenerRemovedLetterDetails.builder().courtDetails(CaseHearingFunctions.buildFrcCourtDetails(caseDetails.getData()))
            .addressee(caseDetails.getData().getCurrentAddressee()).divorceCaseNumber(caseDetails.getData().getDivorceCaseNumber())
            .applicantName(caseDetails.getData().getFullApplicantName()).respondentName(caseDetails.getRespondentFullName())
            .letterDate(DateTimeFormatter.ofPattern(LETTER_DATE_FORMAT).format(LocalDate.now())).caseNumber(caseDetails.getId().toString())
            .intervenerFullName(caseDetails.getData().getCurrentIntervenerChangeDetails().getIntervenerDetails().getIntervenerName()).build();
    ...
    private IntervenerAddedSolicitorLetterDetails generateSolAddedLetterDetails(FinremCaseDetails caseDetails) {
        ...
        return IntervenerAddedSolicitorLetterDetails.builder().courtDetails(CaseHearingFunctions.buildFrcCourtDetails(caseDetails.getData()))
            .addressee(caseDetails.getData().getCurrentAddressee()).divorceCaseNumber(caseDetails.getData().getDivorceCaseNumber())
            .applicantName(caseDetails.getData().getFullApplicantName()).respondentName(caseDetails.getRespondentFullName())
            .letterDate(DateTimeFormatter.ofPattern(LETTER_DATE_FORMAT).format(LocalDate.now())).caseNumber(caseDetails.getId().toString())
            .intervenerFullName(caseDetails.getData().getCurrentIntervenerChangeDetails().getIntervenerDetails().getIntervenerName())
            .intervenerSolicitorFirm(
                caseDetails.getData().getCurrentIntervenerChangeDetails().getIntervenerDetails().getIntervenerOrganisation().getOrganisation()
                    .getOrganisationName()).build();
    }

    private IntervenerRemovedSolicitorLetterDetails generateSolRemovedLetterDetails(FinremCaseDetails caseDetails) {

        return IntervenerRemovedSolicitorLetterDetails.builder().courtDetails(CaseHearingFunctions.buildFrcCourtDetails(caseDetails.getData()))
            .addressee(caseDetails.getData().getCurrentAddressee()).divorceCaseNumber(caseDetails.getData().getDivorceCaseNumber())
            .applicantName(caseDetails.getData().getFullApplicantName()).respondentName(caseDetails.getRespondentFullName())
            .letterDate(DateTimeFormatter.ofPattern(LETTER_DATE_FORMAT).format(LocalDate.now())).caseNumber(caseDetails.getId().toString())
            .intervenerFullName(caseDetails.getData().getCurrentIntervenerChangeDetails().getIntervenerDetails().getIntervenerName())
