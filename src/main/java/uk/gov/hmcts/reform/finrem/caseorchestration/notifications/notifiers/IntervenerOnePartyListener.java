...
public class IntervenerOnePartyListener extends AbstractPartyListener {
    ...
    protected PartySpecificDetails setPartySpecificDetails(SendCorrespondenceEvent event) {
        String email = event.getCaseDetails().getData().getIntervenerOne().getIntervenerSolEmail();
        String name = event.getCaseDetails().getData().getIntervenerOne().getIntervenerSolName();
