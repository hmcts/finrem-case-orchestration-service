...
@Service
@Slf4j
public class ConsentedCreateGeneralOrderSubmittedHandler extends FinremCallbackHandler {

    private final FinremGeneralOrderRaisedCorresponder generalOrderRaisedCorresponder;

    public ConsentedCreateGeneralOrderSubmittedHandler(FinremCaseDetailsMapper finremCaseDetailsMapper,
    ...
    public boolean canHandle(CallbackType callbackType, CaseType caseType, EventType eventType) {
        return SUBMITTED.equals(callbackType) && CaseType.CONSENTED.equals(caseType) && GENERAL_ORDER.equals(eventType);
    }

    @Override
    public GenericAboutToStartOrSubmitCallbackResponse<FinremCaseData> handle(
        FinremCallbackRequest callbackRequestWithFinremCaseDetails, String userAuthorisation) {
        log.info("Consented Create General Order submitted callback for case id: {}",
